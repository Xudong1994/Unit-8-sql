...
SELECT name,Bookings.memid,slots,membercost, guestcost
FROM Bookings
INNER JOIN Facilities ON Bookings.facid = Facilities.facid
INNER JOIN Members ON Members.memid = Bookings.memid
WHERE starttime LIKE '2012-09-14%'
AND Bookings.slots * membercost >=30
OR Bookings.slots * guestcost >=30
...
select name,Bookings.memid,concat(surname, ` `, firstname),membercost,guestcost,slots
from Bookings
Inner join (select
...
where Bookings.slots * membercost >=30
OR Bookings.slots * guestcost >=30
order by  membercost desc
